...
- **핵심 구조**: **EIIE(Ensemble of Identical Independent Evaluators)** 기반 정책 신경망(CNN)
- **입력 데이터**: 산업 ETF 가격, 시가총액, 재무제표 항목, 주요 거시지표
- **운용 방식**: 시점별 포트폴리오 가중치 동적 조정
- **평가 방식**: Buy and Hold 전략과 성과 비교

---

## 서론

전통적인 기술적 지표 중심 투자 전략은 시장 노이즈와 구조 변화에 취약하며, 단순 가격 패턴만으로는 변화 원인까지 설명하기 어렵습니다.
본 프로젝트는 이 한계를 보완하기 위해 재무지표와 거시지표를 함께 활용하고, 강화학습으로 시점별 포트폴리오 비중을 동적으로 조정하는 접근을 채택합니다.
