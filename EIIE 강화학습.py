...
'''
DRLAgent.train_model(model, episodes=200)
...
# 예시: df_portfolio_train은 'time' 컬럼에 날짜 정보를 가지고 있다고 가정합니다.
# ubah_values와 eiiE_values는 각각 UBAH와 EIIE 결과로 나온 포트폴리오 가치 리스트입니다.

# 중복 제거 후 날짜를 가져오고, timewin 만큼 날짜를 조정 (timewin - 1 인덱스부터 시작)
train_time = df_portfolio_train["time"].unique()
train_time = train_time[timewin - 1 :]

# 예시: UBAH와 EIIE의 결과가 리스트 형태라고 가정합니다.
ubah_values = UBAH_results['train']['value']
eiiE_values = EIIE_results['train']['value']

# "time" 컬럼을 인덱스로 사용하여 Series 생성
...
# Sharpe Ratio 계산 (리스크 프리 이자율 및 periods 지정)
sharpe_ubah = qt.stats.sharpe(ubah_return, rf=rf, periods=periods)
sharpe_eiiE = qt.stats.sharpe(eiiE_return, rf=rf, periods=periods)

# 최대 낙폭(MDD) 계산
mdd_ubah = qt.stats.max_drawdown(ubah_price)
mdd_eiiE = qt.stats.max_drawdown(eiiE_price)

# 소르티노 비율 계산 (리스크 프리 이자율 및 periods 지정)
sortino_ubah = qt.stats.sortino(ubah_return, rf=rf, periods=periods)
sortino_eiiE = qt.stats.sortino(eiiE_return, rf=rf, periods=periods)

# 연평균 복리 수익률(CAGR) 계산
cagr_ubah = qt.stats.cagr(ubah_price, periods=periods)
cagr_eiiE = qt.stats.cagr(eiiE_price, periods=periods)

# 오메가 비율(Omega Ratio) 계산 
# rf 인자 대신 연 2%의 최소 요구 수익률(required_return=0.02)을 적용합니다.
omega_ubah = qt.stats.omega(ubah_return, required_return=0.02, periods=periods)
omega_eiiE = qt.stats.omega(eiiE_return, required_return=0.02, periods=periods)

print("UBAH Sharpe Ratio:", sharpe_ubah)
print("EIIE Sharpe Ratio:", sharpe_eiiE)
print("UBAH Maximum Drawdown:", mdd_ubah)
print("EIIE Maximum Drawdown:", mdd_eiiE)
print("UBAH Sortino Ratio:", sortino_ubah)
print("EIIE Sortino Ratio:", sortino_eiiE)
print("UBAH CAGR:", cagr_ubah)
print("EIIE CAGR:", cagr_eiiE)
print("UBAH Omega Ratio:", omega_ubah)
print("EIIE Omega Ratio:", omega_eiiE)

# --------------------------
# 테스트 데이터에 대한 계산
# --------------------------

# 중복 제거 후 날짜를 가져오고, timewin 만큼 날짜를 조정
...
# Sharpe Ratio 계산 (리스크 프리 이자율 및 periods 지정)
sharpe_ubah_test = qt.stats.sharpe(ubah_test_return, rf=rf, periods=periods)
sharpe_eiiE_test = qt.stats.sharpe(eiiE_test_return, rf=rf, periods=periods)

# 최대 낙폭(MDD) 계산
mdd_ubah_test = qt.stats.max_drawdown(ubah_test_price)
mdd_eiiE_test = qt.stats.max_drawdown(eiiE_test_price)

# 소르티노 비율 계산 (리스크 프리 이자율 및 periods 지정)
sortino_ubah_test = qt.stats.sortino(ubah_test_return, rf=rf, periods=periods)
sortino_eiiE_test = qt.stats.sortino(eiiE_test_return, rf=rf, periods=periods)

# 연평균 복리 수익률(CAGR) 계산
cagr_ubah_test = qt.stats.cagr(ubah_test_price, periods=periods)
cagr_eiiE_test = qt.stats.cagr(eiiE_test_price, periods=periods)

# 오메가 비율(Omega Ratio) 계산 (required_return=0.02 적용)
omega_ubah_test = qt.stats.omega(ubah_test_return, required_return=0.02, periods=periods)
omega_eiiE_test = qt.stats.omega(eiiE_test_return, required_return=0.02, periods=periods)

print("UBAH Sharpe Ratio test:", sharpe_ubah_test)
print("EIIE Sharpe Ratio test:", sharpe_eiiE_test)
print("UBAH Maximum Drawdown test:", mdd_ubah_test)
print("EIIE Maximum Drawdown test:", mdd_eiiE_test)
print("UBAH Sortino Ratio test:", sortino_ubah_test)
print("EIIE Sortino Ratio test:", sortino_eiiE_test)
print("UBAH CAGR test:", cagr_ubah_test)
print("EIIE CAGR test:", cagr_eiiE_test)
print("UBAH Omega Ratio test:", omega_ubah_test)
print("EIIE Omega Ratio test:", omega_eiiE_test)





# DataFrame 생성 후 CSV 저장 (훈련 데이터)
train_df = pd.DataFrame({
    'time': train_time,
    'ubah_price': ubah_price.values,
    'eiiE_price': eiiE_price.values
})
train_df.to_excel("train_portfolio_data.xlsx", index=False)



# DataFrame 생성 후 CSV 저장 (테스트 데이터)
test_df = pd.DataFrame({
    'time': test_time,
    'ubah_test_price': ubah_test_price.values,
    'eiiE_test_price': eiiE_test_price.values
})
test_df.to_excel("test_portfolio_data.xlsx", index=False)
